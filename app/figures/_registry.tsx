...
export const FIGURES: Record<string, FigureDef> = {
  ...
  "adoption-curves": {
    ...
    render: () => ({
      ...
      caption:
        "Adoption re-based to years since each technology's own mandate (federal series) or introduction (organic series), first 12 years; the key below the chart gives each clock's calendar date. Populations differ by series: federal HTTPS = live parent .gov domains (IFP-computed from GSA's archived weekly scans); PIV = federal civilian users (OMB FISMA reports); cloud = share of the 24 CFO Act agencies holding ≥1 agency ATO on a FedRAMP-authorized service (IFP, marketplace snapshot 2026-06-12 — a floor, since withdrawn services aren't counted). Vermilion pair — federal LLM access from the same IFP evidence base, on a clock starting at the ChatGPT release: solid = corroborated floor (workers weighted by each agency's best corroborated share; tier-only agencies count as zero), dashed = bullish availability reading (an agency's full AI-eligible workforce counts from its first dated corroborated rollout — availability, not measured use). Evidence dates lag rollouts; IFP assessments, not OMB data. The vermilion rule marks the LLM-access mandate arriving 2.6 years into that clock. Mandate lag collapsed across generations — HTTPS was mandated ~21 years after entering commercial use (16.8% of parent .gov domains enforced it at mandate), IaaS cloud ~5 years after EC2, LLM access 2.6 years after ChatGPT — so each mandate clock starts long after its technology existed; the key notes each lag. IFP analysis; external baselines verified 2026-07-06.",
    }),
  },
