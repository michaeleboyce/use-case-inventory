...
export const FIGURES: Record<string, FigureDef> = {
  ...
  "adoption-curves": {
    ...
    render: () => ({
      ...
      caption:
        "Apples to apples: every series re-based to years since its TECHNOLOGY entered use (year 0 — HTTPS ≈1994 Netscape SSL; smart-card credentials ≈1995; IaaS 2006 EC2 beta; ChatGPT Nov 2022), first 25 years, with each federal mandate marked as a colored rule on that same clock: M-15-13 at yr 20.6, HSPD-12 at yr 9.2, the FedRAMP memo at yr 5.3, the LLM-access mandate at yr 2.6. Each successive mandate arrives earlier in its technology's life; a curve left of its rule is pre-mandate adoption, right of it is post-mandate. Populations differ by series: federal HTTPS = live parent .gov domains (IFP-computed from GSA's archived weekly scans); PIV = federal civilian users (OMB FISMA reports); cloud = share of the 24 CFO Act agencies holding ≥1 agency ATO on a FedRAMP-authorized service (IFP, marketplace snapshot 2026-06-12 — a floor, since withdrawn services aren't counted); gray context = US-household computer adoption on its own arrival clock (Our World in Data). Vermilion pair — federal LLM access from the same IFP evidence base: solid = corroborated floor (workers weighted by each agency's best corroborated share; tier-only agencies count as zero), dashed = bullish availability reading (an agency's full AI-eligible workforce counts from its first dated corroborated rollout — availability, not measured use). Evidence dates lag rollouts; IFP assessments, not OMB data. The interactive chart at /adoption also offers the original mandate-clock view (years since each mandate — post-mandate response speed). IFP analysis; external baselines verified 2026-07-06.",
    }),
  },
